      ...
      <el-card class="notice-card" shadow="never">
        <template #header>
          <div class="card-header">
            ...
            <el-button type="primary" link @click="$router.push('/lol/news')">更多</el-button>
          </div>
        </template>
    ...
    <div class="section-container">
      ...
      <div class="feature-grid">
        ...
        <el-card class="feature-card">
          ...
          <h3>赛事新闻</h3>
          <p>了解最新的英雄联盟赛事动态和新闻</p>
        </el-card>
      </div>
      <div class="coming-soon">
        <el-empty description="英雄联盟板块正在开发中，敬请期待！" :image-size="120" />
      </div>
    </div>
  </div>
</template>
...
const bannerList = ref([
  { image: 'https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&w=1200&q=80', title: '2026年高校英雄联盟春季巅峰赛' },
  { image: 'https://images.unsplash.com/photo-1511512578047-dfb367046420?auto=format&fit=crop&w=1200&q=80', title: '英雄联盟全服集结：寻找最强校园战队' }
]);
...
const fetchData = async () => {
  ...
  try {
    // 动态获取 Banner
    try {
      const bannerRes = await request.get('/banner/active', { params: { gameProject: 'LOL' } });
    ...
    const [newsRes, noticeRes] = await Promise.all([
      request.get('/news/list', { params: { gameProject: 'LOL' } }),
      request.get('/notice/list', { params: { gameProject: 'LOL' } })
    ]);
...
const goNewsDetail = (item) => {
  if (item.type === 'news') {
    router.push(`/lol/news/${item.id}`);
  } else {
    router.push(`/lol/notice/${item.id}`);
  }
};
...
onMounted(() => {
  console.log('LOL home page mounted');
  fetchData();
});
</script>
